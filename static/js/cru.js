const $cru = e => document.querySelector(e),
    ...
    $C = (e = !1) => {
        if (e)
            for (let t of Object.keys(e)) $cruConfig[t] = e[t];
        $cruLoadEvents()
    },
    $cruLoadEvents = () => {
    ...
    $cruLoadContainer = async e => {
        ...
        const t = e.closest("[c-container]") || e,
            ...
            i = c ? $cru(c) : t;
        (c || "off" != c) && (c ? i.innerHTML = s : "html" == a && (i.innerHTML = s)), n && $cruConfig.callbacks[n](s, i), $cruLoadEvents()
        ...
        $crus("[c-container]:not(.loaded)").forEach(async e => {
            ...
            e.classList.add("loaded"), e.addEventListener("click", t => $cruLoadContainer(e))
        })
        ...
        const r = e.getAttribute(`c-${t}`),
            ...
            f = !!$ && $cru($);
        n && e.closest(n).remove(), o && e.remove(), i && ($cru(i).outerHTML = L), d && $cru(d).insertAdjacentHTML("beforeend", L), u && $cru(u).insertAdjacentHTML("afterbegin", L), a && $cruLoadContainer(e), f && (f ? f.innerHTML = L : "html" == c && (e.innerHTML = L)), l && $cruConfig.callbacks[l](L, f), $cruLoadEvents(), s && (window.location.href = s)
    }, $cruLoadRequests = () => {
        $crus("[c-delete]:not(.loaded)").forEach(e => {
            e.classList.add("loaded"), e.addEventListener("click", async t => {
                cruRequest(e, "delete")
            })
        ...
        $crus(".c-form:not(.loaded)").forEach(e => {
            e.classList.add("loaded"), e.addEventListener("submit", async t => {
                t.preventDefault();
                ...
                d && ($cru(d).outerHTML = p), n && $cru(n).insertAdjacentHTML("beforeend", p), o && $cru(o).insertAdjacentHTML("afterbegin", p), u && ($cru(u).innerHTML = p), i && e.reset(), l && $cruLoadContainer(e), $ && $cruConfig.callbacks[$]({
                    status: b.status,
                    data: p
                }, e), $cruLoadEvents(), s && (window.location.href = s)
            })
        })
    }, cruFormatURL = (e, t, r) => {
    ...
    }, $cruIsRead = e => ["GET", "HEAD"].includes(e), $cruTypeResponse = async (e, t) => "html" == e ? await t.text() : await t.json();
$C();